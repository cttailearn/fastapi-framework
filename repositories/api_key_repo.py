...
class ApiKeyRepository:
    ...
    async def create_key(self, user_id: int, name: str, key_hash: str, prefix: str) -> int:
    ...
    async def list_keys(self, user_id: int) -> list[dict[str, object]]:
        rows = await self._db.fetchall(
            """
            SELECT id, user_id, name, prefix, created_at, revoked_at, last_used_at
            FROM api_keys
            WHERE user_id=?
            ORDER BY id DESC;
            """.strip(),
    ...
    async def list_all_keys(self) -> list[dict[str, object]]:
        rows = await self._db.fetchall(
            """
            SELECT k.id, k.user_id, u.username, k.name, k.prefix, k.created_at, k.revoked_at, k.last_used_at
            FROM api_keys k
